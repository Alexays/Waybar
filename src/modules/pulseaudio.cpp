...
waybar::modules::Pulseaudio::Pulseaudio(Json::Value config)
  : _config(std::move(config)), _mainloop(nullptr), _mainloop_api(nullptr),
    _context(nullptr), _sinkIdx(0), _volume(0), _muted(false)
{
  ...
  if (pa_context_connect(_context, nullptr, PA_CONTEXT_NOAUTOSPAWN, nullptr) < 0) {
    auto err = pa_strerror(pa_context_errno(_context));
    throw std::runtime_error(fmt::format("pa_context_connect() failed: {}", err));
  }
 ...
 */
void waybar::modules::Pulseaudio::_serverInfoCb(pa_context *context, const pa_server_info *i, void *data)
{
    pa_context_get_sink_info_by_name(context, i->default_sink_name, _sinkInfoCb, data);
}
...
auto waybar::modules::Pulseaudio::update() -> void
{
    ...
    if (_muted) {
      format = _config["format-muted"] ? _config["format-muted"].asString() : format;
