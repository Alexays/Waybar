...
KeyboardState::KeyboardState(const std::string& id, const Bar& bar,
...
auto KeyboardState::update() -> void {
  ...
  for (auto& label_state : label_states) {
    std::string text{fmt::format(fmt::runtime(label_state.format),
...
KeyboardState::operator Gtk::Widget&() { return box_; };
