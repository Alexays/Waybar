...
Memory::Memory(const std::string& id, const Json::Value& config)
  ...
  args_.push_back(Arg{"percentage", std::bind(&Memory::getPercentage, this), STATE | DEFAULT});
  args_.push_back(Arg{"total", std::bind(&Memory::getTotal, this)});
  args_.push_back(Arg{"used", std::bind(&Memory::getUsed, this)});
  args_.push_back(Arg{"avail", std::bind(&Memory::getAvailable, this)});
  args_.push_back(Arg{"", std::bind(&Memory::getTooltipText, this), TOOLTIP});
  thread_ = [this] {
    dp.emit();
...
const std::string Memory::getTooltipText() const { return fmt::format("{}Gb used", getUsed()); }
