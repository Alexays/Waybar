...
std::string luaEscape(const std::string& value) {
  std::string escaped;
...
std::string IPC::buildLuaDispatch(const std::string& dispatcher, const std::string& arg) {
  const std::string escaped = luaEscape(arg);
  if (dispatcher == "workspace") {
    return "/dispatch hl.dsp.focus({ workspace = \"" + escaped + "\" })";
  }
  if (dispatcher == "focusworkspaceoncurrentmonitor") {
    return "/dispatch hl.dsp.focus({ workspace = \"" + escaped + "\", on_current_monitor = true })";
  }
  if (dispatcher == "togglespecialworkspace") {
    if (arg.empty()) {
      return "/dispatch hl.dsp.workspace.toggle_special()";
    }
    return "/dispatch hl.dsp.workspace.toggle_special(\"" + escaped + "\")";
  }

  spdlog::warn("Hyprland IPC: unknown dispatcher '{}', attempting generic hl.dsp format",
               dispatcher);
  return "/dispatch hl.dsp." + dispatcher + "(\"" + escaped + "\")";
}
