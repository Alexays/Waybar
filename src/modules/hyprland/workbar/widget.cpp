...
Widget::Widget() : Gtk::Box(Gtk::ORIENTATION_HORIZONTAL) {
  instance_ = this;
  show_all();
}
...
Widget* Widget::instance() { return instance_; }
...
WorkspaceButton* Widget::workspaceAt(int x, int y) {
  ...
  for (size_t i = 0; i < children.size(); ++i) {
    ...
    if (i + 1 < children.size()) {
      auto* next = dynamic_cast<WorkspaceButton*>(children[i + 1]);
...
void Widget::endDrag() {
  ...
  if (hovered_workspace_) {
    IPC::dispatch("movetoworkspacesilent",
                  std::to_string(hovered_workspace_->id()) + ",address:" + dragged_window_.address);
  }
