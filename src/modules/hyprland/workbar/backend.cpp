...
WorkspaceList Backend::getWorkspaces() {
  ...
  for (const auto& ws : json) {
    ...
    for (const auto& client : clients) {
      ...
      state.windows.push_back({client["address"].asString(), client["class"].asString(),
                               client["title"].asString(), state.id,
                               client["address"].asString() == active_address, state.visible});
    }
