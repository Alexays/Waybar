...
waybar::modules::sway::Workspaces::Workspaces(Bar &bar, Json::Value config)
  : _bar(bar), _config(std::move(config)), _scrolling(false)
{
  ...
  _thread = [this] {
    try {
      ...
      } else if (!_workspaces.empty()) {
        ipcRecvResponse(_ipcEventfd);
      }
      uint32_t len = 0;
      std::lock_guard<std::mutex> lock(_mutex);
      auto str = ipcSingleCommand(_ipcfd, IPC_GET_WORKSPACES, nullptr, &len);
...
void waybar::modules::sway::Workspaces::_addWorkspace(Json::Value node)
{
  ...
  button.signal_clicked().connect([this, pair] {
    try {
      ...
      ipcSingleCommand(_ipcfd, IPC_COMMAND, value.c_str(), &size);
    } catch (const std::exception& e) {
      std::cerr << e.what() << std::endl;
    }
  });
