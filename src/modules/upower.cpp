     ...
     : AIconLabel(config, "upower", id, "{percentage}", 0, true, true, true)
{
  ...
  if (config_["show-icon"].isBool())
    showIcon_ = config_["show-icon"].asBool();
  if (!showIcon_)
    box_.remove(image_);
  // Device user wants
  ...
  if (config_["hide-if-empty"].isBool())
    hideIfEmpty_ = config_["hide-if-empty"].asBool();

  // Tooltip Spacing
  if (config_["tooltip-spacing"].isInt())
    tooltip_spacing_ = config_["tooltip-spacing"].asInt();
  ...
  // Start watching DBUS
  watcherID_ = Gio::DBus::watch_name(Gio::DBus::BusType::SYSTEM,
                                     "org.freedesktop.UPower",
                                     sigc::mem_fun(*this, &UPower::onAppear),
                                     sigc::mem_fun(*this, &UPower::onVanished),
...
static std::string_view secondsToString(const std::chrono::seconds sec) {
  ...
  std::string_view strRet{(ds.count() > 0) ? "{D}d {H}h {M}min" :
                          ...
                          (min.count() > 0) ? "{M}min" : ""};
  spdlog::debug("UPower::secondsToString(). seconds: \"{0}\", minutes: \"{1}\", hours: \"{2}\", \
days: \"{3}\", strRet: \"{4}\"",
...
void UPower::onVanished(const Glib::RefPtr<Gio::DBus::Connection> &conn, const Glib::ustring &name) {
  upRunning_ = false;
}
...
void UPower::setDisplayDevice() {
  ...
  if (nativePath_.empty()) {
    ...
    g_ptr_array_foreach(up_client_get_devices2(upClient_),
                        [](gpointer data,
                           gpointer user_data) {
                          ...
                          if (0 == std::strcmp(upDevice.nativePath,
                                               thisPtr->nativePath_.c_str())) {
                            // Unref current upDevice
                            if (thisPtr->upDevice_.upDevice)
                              g_object_unref(thisPtr->upDevice_.upDevice);
...
void UPower::getUpDeviceInfo(upDevice_output &upDevice_) {
  if (upDevice_.upDevice &&
      G_IS_OBJECT(upDevice_.upDevice)) {
    g_object_get(upDevice_.upDevice,
                 "kind", &upDevice_.kind,
                 "state", &upDevice_.state,
                 "percentage", &upDevice_.percentage,
                 "icon-name", &upDevice_.icon_name,
                 "time-to-empty", &upDevice_.time_empty,
                 "time-to-full", &upDevice_.time_full,
                 "temperature", &upDevice_.temperature,
                 "native-path", &upDevice_.nativePath,
                 "model", &upDevice_.model,
                 NULL);
    spdlog::debug("UPower. getUpDeviceInfo. kind: \"{0}\". state: \"{1}\". percentage: \"{2}\". \
icon_name: \"{3}\". time-to-empty: \"{4}\". time-to-full: \"{5}\". temperature: \"{6}\". \
native_path: \"{7}\". model: \"{8}\"",
                  upDevice_.kind,
                  upDevice_.state,
                  upDevice_.percentage,
                  upDevice_.icon_name,
                  upDevice_.time_empty,
                  upDevice_.time_full,
                  upDevice_.temperature,
                  upDevice_.nativePath,
                  upDevice_.model);
  }
}
...
const Glib::ustring UPower::getText(const upDevice_output &upDevice_, const std::string &format) {
  ...
  if (upDevice_.upDevice) {
    ...
    ret = fmt::format(fmt::runtime(format),
                      ...
                      fmt::arg("model", upDevice_.model),
                      fmt::arg("native-path", upDevice_.nativePath));
  }

  return ret;
}

bool UPower::queryTooltip_cb(int x, int y, bool keyboard_tooltip, const Glib::RefPtr<Gtk::Tooltip> &tooltip) {
  std::lock_guard<std::mutex> guard{mutex_};
  ...
  for (auto pairDev : devices_) {
    ...
    if (pairDev.second.kind != UpDeviceKind::UP_DEVICE_KIND_UNKNOWN &&
      ...
      if (!gtkTheme_->has_icon(iconNameDev))
