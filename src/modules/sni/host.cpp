...
Host::Host(std::size_t id, const Json::Value& config, const Bar& bar,
  ...
  auto parse_list = [](const Json::Value& list, std::unordered_map<std::string, std::size_t>& out) {
...
void Host::itemReady(Item& item) {
  ...
  if (it != items_.end() && (*it)->isReady()) {
    // on_add_ positions the new widget according to the configured order.
    on_add_(*it);
  }
}
...
int Host::resolveOrder(const std::string& key) const {
  // Explicit numeric orders win over the convenience lists.
  if (auto it = orders_.find(key); it != orders_.end()) {
    return it->second;
  }

  // order-left / order-right are folded into the same integer scale: bases far
  // enough from 0 that listed items always end up outside the unconfigured
  // (order 0) middle group, while keeping their relative list positions.
  static constexpr int kLeftBase = std::numeric_limits<int>::min() / 2;
  static constexpr int kRightBase = std::numeric_limits<int>::max() / 2;

  const auto lower = toLowerAscii(key);
  if (lower.empty()) {
    return 0;
  }
  if (auto it = order_left_.find(lower); it != order_left_.end()) {
    return kLeftBase + static_cast<int>(it->second);
  }
  if (auto it = order_right_.find(lower); it != order_right_.end()) {
    return kRightBase - static_cast<int>(order_right_.size() - it->second);
  }
  return 0;
}
