...
void Tray::reorderBox() {
  ...
  // Items sharing an order value (in particular the unconfigured ones, which
  // all get 0) are sorted alphabetically by their stable key, so the tray
  // layout does not depend on the arrival order of the D-Bus signals.
