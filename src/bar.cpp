...
bool waybar::Bar::is_module_area(int x) const {
  ...
  if (x > (int)surface_impl_->getWidth() - right_.get_allocation().get_width()) {
    // in the right box
    return true;
  }
  const int half_center_width = center_.get_allocation().get_width() / 2;
  if (x > (int)surface_impl_->getWidth() / 2 - half_center_width &&
      x < (int)surface_impl_->getWidth() / 2 + half_center_width) {
    // in the center box
    return true;
  }
  return false;
