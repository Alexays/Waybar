...
waybar::services::Clock::Clock(const std::string& id, const Json::Value& config)
    : DBusService(config, "clock", id, "{:%H:%M}"),
      current_time_zone_idx_{0},
      is_calendar_in_tooltip_{false},
      is_timezoned_list_in_tooltip_{false} {
  if (config["timezones"].isArray() && !config["timezones"].empty()) {
    for (const auto& zone_name : config["timezones"]) {
  ...
  } else if (config["timezone"].isString() && !config["timezone"].asString().empty()) {
    try {
      time_zones_.push_back(date::locate_zone(config["timezone"].asString()));
    } catch (const std::exception& e) {
      spdlog::warn("Timezone: {0}. {1}", config["timezone"].asString(), e.what());
    }
  }
  ...
  if (config["tooltip-format"].isString()) {
    std::string trimmed_format = config["tooltip-format"].asString();
  ...
  if (is_calendar_in_tooltip_) {
    if (config[kCalendarPlaceholder]["weeks-pos"].isString()) {
      if (config[kCalendarPlaceholder]["weeks-pos"].asString() == "left") {
        cldWPos_ = WeeksSide::LEFT;
      } else if (config[kCalendarPlaceholder]["weeks-pos"].asString() == "right") {
        cldWPos_ = WeeksSide::RIGHT;
      }
    }
    if (config[kCalendarPlaceholder]["format"]["months"].isString())
      fmtMap_.insert({0, config[kCalendarPlaceholder]["format"]["months"].asString()});
    else
      fmtMap_.insert({0, "{}"});
    if (config[kCalendarPlaceholder]["format"]["days"].isString())
      fmtMap_.insert({2, config[kCalendarPlaceholder]["format"]["days"].asString()});
    else
      fmtMap_.insert({2, "{}"});
    if (config[kCalendarPlaceholder]["format"]["weeks"].isString() &&
        cldWPos_ != WeeksSide::HIDDEN) {
      fmtMap_.insert(
          {4, std::regex_replace(config[kCalendarPlaceholder]["format"]["weeks"].asString(),
                                 std::regex("\\{\\}"),
    ...
    if (config[kCalendarPlaceholder]["format"]["weekdays"].isString())
      fmtMap_.insert({1, config[kCalendarPlaceholder]["format"]["weekdays"].asString()});
    else
      fmtMap_.insert({1, "{}"});
    if (config[kCalendarPlaceholder]["format"]["today"].isString()) {
      fmtMap_.insert({3, config[kCalendarPlaceholder]["format"]["today"].asString()});
      cldBaseDay_ =
          date::year_month_day{date::floor<date::days>(std::chrono::system_clock::now())}.day();
    } else
      fmtMap_.insert({3, "{}"});
    if (config[kCalendarPlaceholder]["mode"].isString()) {
      const std::string cfgMode{(config[kCalendarPlaceholder]["mode"].isString())
                                    ? config[kCalendarPlaceholder]["mode"].asString()
                                    : "month"};
    ...
    if (config[kCalendarPlaceholder]["mode-mon-col"].isInt()) {
      cldMonCols_ = config[kCalendarPlaceholder]["mode-mon-col"].asInt();
      ...
      cldMonCols_ = 1;
    if (config[kCalendarPlaceholder]["on-scroll"].isInt()) {
      cldShift_ = date::months{config[kCalendarPlaceholder]["on-scroll"].asInt()};
    }
  }

  if (config["locale"].isString())
    locale_ = std::locale(config["locale"].asString());
  else
    locale_ = std::locale("");
}

auto waybar::services::Clock::start() -> void {}
auto waybar::services::Clock::stop() -> void {}

const date::time_zone* waybar::services::Clock::current_timezone() {
  return time_zones_[current_time_zone_idx_] ? time_zones_[current_time_zone_idx_]
                                             : date::current_zone();
}

bool waybar::services::Clock::is_timezone_fixed() {
...
auto waybar::services::Clock::get_calendar(const date::zoned_seconds& now,
...
const Glib::ustring& waybar::services::Clock::getLabelText() {
  ...
  if (!is_timezone_fixed()) {
    ...
    labelText_ = fmt::format(locale_, fmt::runtime(format_), localtime);
  } else {
    labelText_ = fmt::format(locale_, fmt::runtime(format_), ztime);
  }

  return labelText_;
}

const Glib::ustring& waybar::services::Clock::getTooltipText() {
  const auto* time_zone = current_timezone();
  auto now = std::chrono::system_clock::now();
  auto ztime = date::zoned_time{time_zone, date::floor<std::chrono::seconds>(now)};
  ...
  if (config_["tooltip-format"].isString()) {
    ...
    tooltipText_ =
        fmt::format(locale_, fmt::runtime(tooltip_format), shifted_ztime,
...
/*Clock actions*/
void waybar::services::Clock::cldModeSwitch() {
  cldMode_ = (cldMode_ == CldMode::YEAR) ? CldMode::MONTH : CldMode::YEAR;
}
void waybar::services::Clock::cldShift_up() {
  cldCurrShift_ += ((cldMode_ == CldMode::YEAR) ? 12 : 1) * cldShift_;
}
void waybar::services::Clock::cldShift_down() {
  cldCurrShift_ -= ((cldMode_ == CldMode::YEAR) ? 12 : 1) * cldShift_;
}
void waybar::services::Clock::tz_up() {
...
void waybar::services::Clock::tz_down() {
  auto nr_zones = time_zones_.size();
...
auto waybar::services::Clock::timezones_text(std::chrono::system_clock::time_point* now)
    -> std::string {
...
auto waybar::services::Clock::first_day_of_week() -> date::weekday {
#ifdef HAVE_LANGINFO_1STDAY
...
auto waybar::services::Clock::doAction(const Glib::ustring& name) -> void {
  if (doActionExists(name))
    (this->*actionMap_[name])();
  else
    spdlog::error("Clock. Unsupported action \"{0}\"", name.c_str());
}

bool waybar::services::Clock::doActionExists(const Glib::ustring& name) {
