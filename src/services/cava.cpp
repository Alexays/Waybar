...
waybar::services::Cava::Cava(const std::string& id, const Json::Value& config)
    : DBusService(config, "cava", id, "") {
...
waybar::services::Cava::~Cava() {
  thread_.stop();
  delete plan_;
  plan_ = nullptr;
}

auto waybar::services::Cava::start() -> void {
  thread_ = [this] {
    thread_.sleep_for(fetch_input_delay_);
    input_source_(&audio_data_);
  };
}

auto waybar::services::Cava::stop() -> void {
...
void waybar::services::Cava::pause_resume() {
...
const Glib::ustring& waybar::services::Cava::getLabelText() {
  if (!audio_data_.suspendFlag) {
    silence_ = true;
    ...
    if (!silence_) {
      ...
      if (rePaint_ == 1) {
        ...
        for (int i{0}; i < audio_raw_.number_of_bars; ++i) {
          ...
          labelText_.append(getIcon(
              (audio_raw_.bars[i] > prm_.ascii_range) ? prm_.ascii_range : audio_raw_.bars[i], "",
              prm_.ascii_range + 1));
          if (prm_.bar_delim != 0) labelText_.push_back(prm_.bar_delim);
        }
      }
    } else
      upThreadDelay(frame_time_milsec_, suspend_silence_delay_);
...
// Cava actions
auto waybar::services::Cava::doAction(const Glib::ustring& name) -> void {
  if (doActionExists(name))
    (this->*actionMap_[name])();
  else
    spdlog::error("Cava. Unsupported action \"{0}\"", name.c_str());
}

bool waybar::services::Cava::doActionExists(const Glib::ustring& name) {
  return (actionMap_.find(name) != actionMap_.cend());
}

const Glib::ustring& waybar::services::Cava::doMethod(const Glib::ustring& name) {
  if (methodMap_[name]) {
    return (this->*methodMap_[name])();
  } else {
    static const auto result{Glib::ustring::format("Cava. Unsupported method \"{0}\"", name)};
    return result;
  }
}

const bool waybar::services::Cava::doMethodExists(const Glib::ustring& name) {
  return (methodMap_.find(name) != methodMap_.cend());
}

const Glib::ustring& waybar::services::Cava::getFrameTime() {
  textFrameTime_ = Glib::ustring::format(frame_time_milsec_.count());
  return textFrameTime_;
}
